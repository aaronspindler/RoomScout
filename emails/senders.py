...
def send_invite_email(to_email, invite):
    ...
    html_content = render_to_string('emails/house_invite_template.html', {'invite': invite})
...
def send_inquiry_email(to_email, inquiry):
    ...
    html_content = render_to_string('emails/room_inquiry_template.html', {'inquiry': inquiry})
...
def send_contact_us_email(submitter_email, submitter_subject, submitter_message, ip):
    ...
    html_content = render_to_string('emails/contact_us_template.html', {'email': submitter_email, 'subject': submitter_subject, 'message': submitter_message, 'ip': ip})
...
def send_bill_email(house, bill):
    ...
    if len(to_emails) > 0:
        ...
        html_content = render_to_string('emails/bill_added_template.html', {'bill': bill, 'house': house})
